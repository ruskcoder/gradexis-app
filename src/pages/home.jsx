...
const HomePage = ({f7router}) => {
  ...
  const switchAccount = () => {
    return () => {
      ...
      let accountPicker = f7.dialog.create({
      ...
      window.pickAccount = (username) => {
        store.dispatch('switchUser', users.findIndex((user) => user.username === username)).then(r => r)
        accountPicker.close();
        //f7.emit('userChanged')
  ...
  return (
      <Page name="home">
        ...
        <List
            mediaList
            dividersIos={user.groupLists}
            outlineIos={user.groupLists}
            strongIos={user.groupLists}
            chevronCenter
            // sortable
            // sortableEnabled
            className={`overviewList mod-list mt-fix ${user.groupLists ? "" : "iosRound"}`}
        >
          <ListItem
