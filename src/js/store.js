import { createStore } from 'framework7/lite';
import { login } from './grades-api.js';
import { f7 } from 'framework7-react';
import { Capacitor } from "@capacitor/core";
import storageManager from './storage.js';

const channel4Broadcast = new BroadcastChannel('channel4');
const defaultpfp = "data:image/webp;base64,";
let users = JSON.parse(localStorage.getItem("users"));
let currentUserNumber = parseInt(localStorage.getItem('currentUserNumber'));
if (!users) {
  localStorage.setItem("users", JSON.stringify([]));
  users = [];
}
let isIOS = Capacitor.getPlatform() === 'ios';
if (!currentUserNumber && currentUserNumber !== 0) {
  localStorage.setItem("currentUserNumber", "-1");
  currentUserNumber = -1;
}
if (currentUserNumber >= users.length) {
  currentUserNumber = users.length - 1;
}
const defaultUser = {
  pfp: defaultpfp,
  scheme: 'light',
  theme: '#007aff',
  layout: isIOS ? "ios" : "md",
  term: -1,
  termList: [],
  gradelist: {},
  scoresIncluded: false,
  stream: true,
  anim: true,
  gradesView: 'card',
  roundGrades: false,
  letterGrades: false,
  pageTransition: 'f7-parallax',
  displayAnimations: false,
  matchColorCards: true,
  groupLists: false
};
const randomColors = [
  '#b700ff',
  '#ff00ae',
  '#ff000d',
  '#ff8400',
  '#ffff00',
  '#8cff00',
  '#00ffb3',
  '#00b3ff',
  '#0000ff',
]
const isNative = Capacitor.isNativePlatform();

const updateSW = () => {
  // For web, use broadcast channel
  if (!isNative) {
    channel4Broadcast.postMessage({
      users: store.state.users,
      currentUserNumber: store.state.currentUserNumber
    });
  }
  
  // For both web and mobile, sync to storage for notification handler
  syncToStorage().then(r => r);
};

const syncToStorage = async () => {
  try {
    await storageManager.setItem('users', store.state.users);
    await storageManager.setItem('currentUserNumber', store.state.currentUserNumber);
  } catch (error) {
    console.error('Failed to sync data to storage:', error);
  }
};

const store = createStore({
  state: {
    users: users,
    currentUser: users.length !== 0 ? users[currentUserNumber] : defaultUser,
    currentUserNumber: currentUserNumber,
    scoresIncluded: false,
    useCache: false,
    loaded: false,
    activeButtonIndex: -1,
    session: {}
  },
  getters: {
    users({ state }) {
      return state.users;
    },
    currentUser({ state }) {
      return state.currentUser;
    },
    currentUserNumber({ state }) {
      return state.currentUserNumber;
    },
    classes({ state }) {
      return state.classes;
    },
    session({ state }) {
      return state.session;
    },
    gradelist({ state }) {
      return state.currentUser.gradelist;
    }
  },
  actions: {
    async addUser({ state }, loginData) {
      const userData = await login(loginData);
      if (userData.success !== false) {
        let newUser = { ...defaultUser };
        if (userData.platform === 'powerschool') {
          newUser.stream = false;
          newUser.gradesView = 'list';
        }
        newUser.theme = randomColors[Math.floor(Math.random() * randomColors.length)];
        if (state.currentUserNumber === -1) {
          newUser.theme = '#007aff'; // Default blue theme for first user
        }
        state.users = [...state.users, { ...userData, ...newUser }];
        localStorage.setItem("users", JSON.stringify(state.users));
        updateSW();

        if (state.currentUserNumber === -1) {
          localStorage.setItem('currentUserNumber', 0);
          updateSW();
          state.currentUserNumber = 0;
          state.currentUser = state.users[0];
        }

        return userData;
      } else {
        f7.dialog.alert(userData.message, "Login Failed");
        return false;
      }
    },
    removeUser({ state }, username) {
      state.users = state.users.filter((user) => user.username !== username);
      localStorage.setItem("users", JSON.stringify(state.users));
      updateSW();
      if (state.currentUser.username === username) {
        if (state.users.length === 0) {
          // state.currentUser = {};
          state.currentUserNumber = -1;
          localStorage.setItem('currentUserNumber', -1);
          updateSW();
        } else {
          state.currentUser = state.users[0];
          state.currentUserNumber = 0;
        }
      }
    },
    changeUserData({ state }, { item, value }) {
      // const updatedUser = { ...state.users[userNumber], [item]: value };
      // state.users = [
      //   ...state.users.slice(0, userNumber),
      //   updatedUser,
      //   ...state.users.slice(userNumber + 1)
      // ];
      state.users = state.users.with(state.currentUserNumber, { ...state.users[state.currentUserNumber], [item]: value });
      localStorage.setItem("users", JSON.stringify(state.users));
      updateSW();
      state.currentUser = state.users[state.currentUserNumber];
    },
    setGradelist({ state }, { gradelist }) {
      state.users = state.users.with(
        state.currentUserNumber,
        { ...state.users[state.currentUserNumber], gradelist }
      );
      localStorage.setItem("users", JSON.stringify(state.users));
      updateSW();
      state.currentUser = state.users[state.currentUserNumber];
    },
    setGradelistAdv({ state }, { term, userClass, item, value }) {
      state.users[state.currentUserNumber]['gradelist'][term][userClass][item] = value;
    },
    switchUser({ state }, userNumber) {
      if (userNumber < state.users.length) {
        state.currentUser = state.users[userNumber];
        state.currentUserNumber = userNumber;
      }
      localStorage.setItem('currentUserNumber', state.currentUserNumber);
      updateSW();
      state.useCache = false;
      store.dispatch('setClasses', []);
      store.dispatch('setSession', {});
      f7.setColorTheme(state.currentUser.theme);
      f7.setDarkMode(state.currentUser.scheme === "dark");
    },
    setClasses({ state }, classes) {
      state.classes = classes;
    },
    setTerm({ state }, term) {
      state.currentUser.term = term;
    },
    setSession({ state }, session) {
      state.session = session;
    },
  }
});

export default store;
export { updateSW };